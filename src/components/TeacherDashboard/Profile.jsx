...
const CLOUDINARY_UPLOAD_PRESET = 'teacher_profile_preset'; // must be UNSIGNED
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        const teacherDoc = doc(db, 'teachers', currentUser.uid);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate('/TeacherPage'); // ✅ Redirect after save
      }, 1000);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen text-black p-16 bg-[#FDFBF7] rounded-3xl overflow-hidden shadow-sm">
      {/* Back Button */}
      <button
        onClick={() => navigate('/TeacherPage')} // ✅ Also use navigate here
