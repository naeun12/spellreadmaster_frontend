// src/components/TeacherRecentActivities.jsx
...
const RecentActivities = () => {
  ...
  const loadTeacherActivities = async () => {
    try {
      ...
      // Optional: include teacher's own activities
      const teacherActivityQ = query(
        collection(db, 'teachers', user.uid, 'activity'),
        orderBy('timestamp', 'desc')
      );
      const teacherActivitySnap = await getDocs(teacherActivityQ);
      teacherActivitySnap.docs.forEach((doc) => {
        const data = doc.data();
        allActivities.push({
          id: doc.id,
          userId: user.uid,
  ...
  // Rest of the logic is IDENTICAL to admin version
        ...
        <div className="flex justify-between items-center">
          ...
          {activities.length > 0 && (
            <button
              ...
              onClick={() => window.location.href = '/TeacherPage/monitor-students'}
            >
              View All
            </button>
          )}
      ...
      <div className="p-6">
        {activities.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-400 mt-1">Student activities will appear here</p>
          </div>
        ) : (
